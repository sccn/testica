...
for algo=1:length(newalgs)
    ...
    allpmi(:,algo) = pmi2.PMI(datasetrange); % 1 Nats = 2*log2(exp(1)) = 1.4427 bits
...
for i=1:length(alldip10), 
    plot(mean(allmir(:,i))/1000, alldip5(i), '.', 'color', 'k', 'markersize', 18); 
    %if i < 13
...
if plotmir
    ...
    xlabel('Pairwise Mutual Information');
end;
